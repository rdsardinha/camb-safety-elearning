...
function truncate_to_lines($content, $line_limit = 1)
{
	// Explode the content into lines
	$lines = explode("\n", wordwrap(strip_tags($content), 100, "\n"));
	// Get the first three lines
	$truncated_lines = array_slice($lines, 0, $line_limit);
...
/**
 * Modify the main query on the frontend archive page to show more posts.
 *
 * By default WordPress shows 10 posts per page on the archive page. This function
 * changes that to 12 posts per page.
 *
 * @param WP_Query $query The main query object.
 */
function custom_archive_posts_per_page($query)
{
	// Only modify the main query on the frontend archive page
	if (!is_admin() && $query->is_main_query() && is_post_type_archive('echo_videos')) {
 ...
 * 
 */
add_action('wp_footer', 'copy_text');
function copy_text()
{ ?>
	<script>
		function copyText() {
